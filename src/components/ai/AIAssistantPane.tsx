...
export const AIAssistantPane: React.FC = () => {
  ...
  const [messages, setMessages] = useState<AIMessage[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I am your **Nexora AI Assistant**. I can write code, explain components, debug errors, and suggest improvements. What would you like to build today?",
  ...
  const handleSendMessage = async (customPrompt?: string) => {
    ...
    try {
      const res = await fetch("/api/ai/assistant", {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            allFiles: currentProject?.files.map((f) => ({
              path: f.path,
              language: f.language,
              isFolder: f.isFolder,
            })),
      ...
      if (!res.ok) {
        throw new Error(data.error || "AI Assistant service error");
      }
  ...
  const extractAndApplyCode = (content: string) => {
    ...
    if (match && match[1]) {
      updateFileContent(activeFile.id, match[1]);
      runPreview();
    } else {
    ...
    <div className="h-full flex flex-col bg-slate-900 border-r border-slate-800 text-slate-300 text-xs select-none">
      ...
      <div className="p-2 border-b border-slate-800/80 bg-slate-950/40 flex items-center gap-1.5 overflow-x-auto scrollbar-none shrink-0">
        {quickPrompts.map((qp, idx) => (
          <button
            ...
            disabled={isLoading}
            className="px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800 hover:border-indigo-500/80 hover:text-indigo-300 text-[11px] whitespace-nowrap transition-all"
          >
            {qp.label}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.map((msg, idx) => {
          ...
          return (
            ...
            >
              {!isUser && (
                <div className="w-6 h-6 rounded-lg bg-indigo-600 flex items-center justify-center text-white shrink-0 mt-0.5 shadow">
              ...
              >
                ...
                {!isUser && (
                  <div className="flex items-center justify-end gap-1 pt-1 border-t border-slate-800/60">
                    ...
                    {hasCode && activeFile && (
                      <button
                        onClick={() => extractAndApplyCode(msg.content)}
                        title="Apply code snippet to current active file"
                        className="flex items-center gap-1 px-2 py-0.5 rounded bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white text-[10px] font-medium transition-all"
                      >
        ...
        {isLoading && (
          <div className="flex items-center gap-2 text-indigo-400 text-xs p-2 bg-slate-950 rounded-xl border border-slate-800 max-w-[200px]">
            <Sparkles className="w-4 h-4 animate-spin text-indigo-400" />
            <span>AI is crafting code...</span>
          </div>
        )}
      ...
      <div className="p-3 bg-slate-950 border-t border-slate-800">
        ...
        >
          <input
            type="text"
            placeholder="Ask AI anything or request code changes..."
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            disabled={isLoading}
            className="flex-1 px-3 py-2 text-xs bg-slate-900 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-indigo-500 placeholder-slate-600"
          />
          <button
            type="submit"
            disabled={!inputValue.trim() || isLoading}
            className="p-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 rounded-xl text-white transition-all shadow-md shadow-indigo-600/30"
          >
